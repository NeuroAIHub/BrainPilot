import { defineMessages } from "../types";

export default defineMessages(
  {
    "files.aria.sidebar": "工作区文件",
    "files.aria.resizeSidebar": "调整文件栏宽度",
    "files.eyebrow.workspace": "工作区",
    "files.title": "文件",
    "files.selectedCount": "已选 {count} 个",
    "files.packing": "打包中",
    "files.download": "下载",
    "files.aria.clearSelection": "清除文件选择",
    "files.aria.refresh": "刷新文件",
    "files.aria.close": "关闭文件",
    "files.live": "实时后端",
    "files.offline": "沙盒离线",
    "files.aria.tree": "文件树",
    "files.selectForDownload": "选择 {name} 以下载",
    "files.error.notRunning": "Sandbox 未运行，无法读取文件。",
    "files.error.notRunningHint": "先新建或打开一个对话以启动 Sandbox，然后再打开文件面板浏览工作区。",
    "files.error.notRunningCta": "关闭文件面板",
    "files.error.loadFailed": "加载文件列表失败",
    "files.error.downloadFailed": "下载文件失败",
    "files.error.refreshFailed": "刷新文件失败",
    "files.error.previewFailed": "无法预览文件",
    "files.error.loadPreviewFailed": "无法加载文件预览",
    "files.error.invalidLink": "该文件链接不在可访问的工作区中。",
    "files.error.linkNotFound": "链接指向的文件不存在或当前不可访问。",
    "files.error.linkNotFile": "该链接指向的是文件夹，而不是文件。",
    "files.error.uploadFailed": "上传文件失败",
    "files.error.tooLarge": "文件过大：{size}，超过上限 {limit}（可用环境变量 BP_UPLOAD_MAX_BYTES 调整）",
    "files.error.tooLargeLimitOnly": "文件过大：超过上限 {limit}（可用环境变量 BP_UPLOAD_MAX_BYTES 调整）",
    "files.aria.dismissError": "关闭错误提示",
    "files.delete": "删除",
    "files.deleting": "删除中",
    "files.confirmDelete": "确认删除 {name} 吗？",
    "files.confirmDeleteBatch": "确认删除已选的 {count} 项吗？",
    "files.error.deleteFailed": "删除失败",
    "files.aria.delete": "删除 {name}",
    "files.aria.deleteSelected": "删除已选文件",
    "files.tier.workspace": "当前会话",
    "files.tier.workspaceHint": "本会话的工作区，agent 在此读写；随会话结束而清理。",
    "files.tier.data": "持久库",
    "files.tier.dataHint": "跨会话持久保存，可在其他会话复用；适合数据集、参考资料。",
    "files.tier.empty": "（空）",
    "files.aria.uploadToData": "上传到持久库",
    "files.preview.aria": "文件预览",
    "files.preview.ariaResize": "调整预览宽度",
    "files.preview.eyebrow": "预览",
    "files.preview.downloading": "下载中",
    "files.preview.download": "下载",
    "files.preview.useInConversation": "用于对话",
    "files.preview.restore": "还原预览",
    "files.preview.maximize": "最大化预览",
    "files.preview.close": "关闭预览",
    "files.preview.path": "路径",
    "files.preview.size": "大小",
    "files.preview.modified": "修改时间",
    "files.preview.mode": "权限",
    "files.preview.restoredVersion": "显示的是 {time} 恢复后重新加载的版本",
    "files.preview.restoredMissing": "该文件在恢复后的工作区中已不存在。",
    "files.preview.tooLarge": "该文件超过内嵌预览大小上限，可下载后查看。",
    "files.preview.missing": "该文件未包含在本次演示包中。",
    "files.preview.unreadable": "打包时无法读取该文件（可能路径无效或不在当前沙箱中）。",
    "files.preview.loadingImage": "正在加载图片…",
    "files.preview.loadingPdf": "正在加载 PDF…",
    "files.preview.notPreviewable": "该文件类型可下载，但不支持内嵌预览。",
    "files.preview.loading": "正在加载预览…",
    "files.editor.preview": "预览",
    "files.editor.edit": "编辑",
    "files.editor.save": "保存",
    "files.editor.saving": "保存中",
    "files.editor.aria": "编辑 {name}",
    "files.editor.saveFailed": "保存文件失败",
    "files.editor.confirmDiscard": "此文件有未保存的更改。确定要放弃吗？",
    "files.editor.conflict": "此文件在编辑期间已被其他进程修改。请重新加载最新版本，或确认覆盖。",
    "files.editor.reload": "重新加载",
    "files.editor.overwrite": "覆盖文件",
  },
  {
    "files.aria.sidebar": "Workspace files",
    "files.aria.resizeSidebar": "Resize files sidebar",
    "files.eyebrow.workspace": "Workspace",
    "files.title": "Files",
    "files.selectedCount": "{count} selected",
    "files.packing": "Packing",
    "files.download": "Download",
    "files.aria.clearSelection": "Clear file selection",
    "files.aria.refresh": "Refresh files",
    "files.aria.close": "Close files",
    "files.live": "live backend",
    "files.offline": "sandbox offline",
    "files.aria.tree": "File tree",
    "files.selectForDownload": "Select {name} for download",
    "files.error.notRunning": "Sandbox is not running; cannot read files.",
    "files.error.notRunningHint": "Start or open a conversation so the Sandbox is running, then reopen Files to browse the workspace.",
    "files.error.notRunningCta": "Close Files",
    "files.error.loadFailed": "Failed to load file list",
    "files.error.downloadFailed": "Failed to download file",
    "files.error.refreshFailed": "Failed to refresh files",
    "files.error.previewFailed": "Unable to preview file",
    "files.error.loadPreviewFailed": "Unable to load file preview",
    "files.error.invalidLink": "This file link is outside the accessible workspace.",
    "files.error.linkNotFound": "The linked file does not exist or is not currently accessible.",
    "files.error.linkNotFile": "The link points to a folder, not a file.",
    "files.error.uploadFailed": "Failed to upload file",
    "files.error.tooLarge": "File too large: {size} exceeds the {limit} limit (adjust via BP_UPLOAD_MAX_BYTES)",
    "files.error.tooLargeLimitOnly": "File too large: exceeds the {limit} limit (adjust via BP_UPLOAD_MAX_BYTES)",
    "files.aria.dismissError": "Dismiss error",
    "files.delete": "Delete",
    "files.deleting": "Deleting",
    "files.confirmDelete": "Delete {name}?",
    "files.confirmDeleteBatch": "Delete {count} selected items?",
    "files.error.deleteFailed": "Delete failed",
    "files.aria.delete": "Delete {name}",
    "files.aria.deleteSelected": "Delete selected files",
    "files.tier.workspace": "Current session",
    "files.tier.workspaceHint": "This session's workspace, where the agent reads and writes; cleared when the session ends.",
    "files.tier.data": "Persistent library",
    "files.tier.dataHint": "Persists across sessions and can be reused in others; good for datasets and reference material.",
    "files.tier.empty": "(empty)",
    "files.aria.uploadToData": "Upload to persistent library",
    "files.preview.aria": "File preview",
    "files.preview.ariaResize": "Resize file preview",
    "files.preview.eyebrow": "Preview",
    "files.preview.downloading": "Downloading",
    "files.preview.download": "Download",
    "files.preview.useInConversation": "Use in conversation",
    "files.preview.restore": "Restore preview",
    "files.preview.maximize": "Maximize preview",
    "files.preview.close": "Close preview",
    "files.preview.path": "Path",
    "files.preview.size": "Size",
    "files.preview.modified": "Modified",
    "files.preview.mode": "Mode",
    "files.preview.restoredVersion": "Showing the version reloaded after the restore at {time}",
    "files.preview.restoredMissing": "This file no longer exists in the restored workspace.",
    "files.preview.tooLarge": "This file is larger than the inline preview limit; download it to view.",
    "files.preview.missing": "This file is not included in this demo bundle.",
    "files.preview.unreadable": "This file could not be read when the demo was packaged (path invalid or not in the current sandbox).",
    "files.preview.loadingImage": "Loading image...",
    "files.preview.loadingPdf": "Loading PDF...",
    "files.preview.notPreviewable": "This file type is available for download but is not previewed inline.",
    "files.preview.loading": "Loading preview...",
    "files.editor.preview": "Preview",
    "files.editor.edit": "Edit",
    "files.editor.save": "Save",
    "files.editor.saving": "Saving",
    "files.editor.aria": "Edit {name}",
    "files.editor.saveFailed": "Failed to save file",
    "files.editor.confirmDiscard": "This file has unsaved changes. Discard them?",
    "files.editor.conflict": "This file changed outside the editor. Reload the latest version or confirm that you want to overwrite it.",
    "files.editor.reload": "Reload",
    "files.editor.overwrite": "Overwrite file",
  },
);
