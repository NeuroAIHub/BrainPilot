...
interface Props { query: string; onCount: (count: number) => void; onQueryChange?: (query: string) => void; refreshSignal?: number; }
...
export function DatasetMarketplace({ query, onCount, onQueryChange, refreshSignal }: Props) {
  const t = useT();
  ...
  const [selectionId, setSelectionId] = useState("full");
  const [credentials, setCredentials] = useState<Record<string, string>>({});
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState(false);
  const startingRef = useRef(false);
  const [initialFilters] = useState(() => {
    try { const value = JSON.parse(sessionStorage.getItem("brainpilot-dataset-filters") ?? "{}"); return value && typeof value === "object" && !Array.isArray(value) ? value as Record<string, unknown> : {}; } catch { return {}; }
  });
  const [modality, setModality] = useState(typeof initialFilters.modality === "string" ? initialFilters.modality : "all");
  const [access, setAccess] = useState(typeof initialFilters.access === "string" && ["all", "automatic", "provider", "credentials", "application"].includes(initialFilters.access) ? initialFilters.access : "all");
  const [domain, setDomain] = useState(typeof initialFilters.domain === "string" && initialFilters.domain in DATASET_DOMAINS ? initialFilters.domain : "all");
  const [sort, setSort] = useState(typeof initialFilters.sort === "string" && ["catalog", "samples", "name"].includes(initialFilters.sort) ? initialFilters.sort : "catalog");
  const [compact, setCompact] = useState(initialFilters.compact === true);
  useEffect(() => { try { sessionStorage.setItem("brainpilot-dataset-filters", JSON.stringify({ modality, access, domain, sort, compact })); } catch { /* Storage is optional. */ } }, [modality, access, domain, sort, compact]);
  ...
  const closeDetails = useCallback(() => { setSelectedId(null); setComparing(false); setCredentials({}); }, []);
  ...
  const reset = () => { setModality("all"); setAccess("all"); setDomain("all"); onQueryChange?.(""); };
  const showDetails = (id: string, option?: string) => {
    setSelectedId(id); setComparing(false); setCredentials({}); setError(null); setCopied(null);
    setSelectionId(option ?? catalog.find((entry) => entry.id === id)?.downloadOptions?.[0]?.id ?? "full");
  };
  ...
  const start = async (entry: DatasetCatalogEntry) => {
    ...
    try {
      updateJob(await api.datasets.download(entry.id, entry.id === selectedId ? credentials : {}, entry.id === selectedId ? selectionId : "full"));
  ...
  return <>
    ...
    <div className="dataset-toolbar">
      ...
      <button className="plugin-card__button plugin-card__button--ghost" aria-pressed={compact} onClick={() => setCompact((value) => !value)} type="button">{compact ? <LayoutGrid size={15} /> : <List size={15} />}{t(compact ? "datasets.cards" : "datasets.compact")}</button>
    </div>
    ...
    {error && !selected ? <div role="alert" className="plugin-market__notice plugin-market__notice--error"><span>{error}</span><button onClick={() => void load()} type="button">{t("marketplace.retry")}</button></div> : null}
