 ...
 * live Chat shows), plus terminal ask_user cards (answered or cancelled;
 * AskUserCard is a read-only record since #272). Drop: reasoning, tool
 * calls/results, hook diagnostics, the auto_retry card and pending ask_user
...
export function isDemoConversational(m: ChatMessage): boolean {
  ...
  // Terminal ask_user records are meaningful in replay; live pending prompts
  // are not.
  if (m.kind === "ask_user") {
    const status = m.askUser?.status
      ?? (m.askUser?.answer === undefined ? "pending" : "answered");
